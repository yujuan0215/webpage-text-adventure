...
感謝你花時間了解我！這個文字冒險遊戲本身就是我創意的一個體現。`,
    ...
    displayCourses(semester) {
        ...
        const courseList = courses.map(course => `<div class="course-item">${course}</div>`).join('');
    ...
    handleNumberKey(number) {
        ...
        if (!currentScene)
            return;
        const visibleChoices = currentScene.choices.filter(choice => !choice.condition || choice.condition());
